...
<div class="max-w-7xl mx-auto items-center flex flex-col">
    <div class="grid grid-cols-2 gap-1 p-10 bg-white border rounded-xl shadow-lg w-full my-10">
        <div class="flex flex-row items-end" title="Laragon">
            ...
            <a class="text-orange-600 hover:underline hover:decoration font-semibold py-2 inline-block align-text-bottom"
               title="Getting Started" href="https://laragon.org/docs" target="_blank">.DOC</a>
        </div>

        <div class="flex flex-col items-end">
            <div><?php print($_SERVER['SERVER_SOFTWARE']); ?></div>
            <div>PHP version: <?php print phpversion(); ?> <span><a
                            class="text-orange-600 hover:underline hover:decoration font-semibold" title="phpinfo()"
                            href="/?q=info" target="_blank">info</a></span></div>
            <div>Document Root: <?php print ($_SERVER['DOCUMENT_ROOT']); ?></div>
        </div>
    </div>

    <div x-data="collection" x-init="getRecords()"
         class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 grid-flow-rows p-10 bg-white border rounded-xl shadow-lg w-full break-normal">
        <template x-for="(item,index) in siteuri" :key="item.id">
            <div class="flex items-center justify-between rounded-lg shadow hover:bg-indigo-100 hover:shadow-lg hover:rounded transition duration-150 ease-in-out transform hover:scale-105 ">
                <a class="rounded-lg py-4 px-6 w-full block text-orange-600 hover:text-black font-semibold break-normal"
                   target="_blank" x-bind:href="item.link" x-text="item.name"></a>
                ...
                <svg class="w-12 h-12 p-3 fill-orange-600" :class="{ 'hidden': item.tech === 'wordpress' }"
                     viewBox="0 0 93.936 93.936" xml:space="preserve">
                            <g>
